...
async function build() {
  ...
  if (profile === "release") {
    const wasm = await Deno.readFile(
      path.resolve(LoroWasmDir, "bundler", "loro_wasm_essential_bg.wasm"),
    );
...
async function buildTarget(target: string) {
  ...
  const cmd = `wasm-bindgen --weak-refs --target ${target} --out-dir ${target} ../../target/wasm32-unknown-unknown/${profileDir}/loro_wasm_essential.wasm`;
  console.log(">", cmd);
  await Deno.run({ cmd: cmd.split(" "), cwd: LoroWasmDir }).status();
  console.log();
  ...
  if (target === "nodejs") {
    ...
    const wasm = await Deno.readTextFile(
      path.resolve(targetDirPath, "loro_wasm_essential.js"),
    );
